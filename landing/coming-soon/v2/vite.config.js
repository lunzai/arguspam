...
const jsToBottomNoModule = () => {
  return {
    ...
    transformIndexHtml(html) {
      // Remove crossorigin and type="module", keep as regular script
      // This allows vendor scripts to be loaded in footer before main.js executes
...
const replaceEnvVariables = (mode) => {
  return {
    ...
    transformIndexHtml(html) {
      ...
      // Replace GTM_ID with actual value from .env
      if (env.GTM_ID) {
        html = html.replace(/GTM_ID/g, env.GTM_ID);
        console.log(`✅ Replaced GTM_ID with ${env.GTM_ID}`);
      } else {
        console.warn('⚠️ GTM_ID not found in .env file');
      }
